...
重要な節目だけを記録する。通常の細かな修正は各項目に追記する。

| 日付 | バージョン | メモ |
| --- | --- | --- |
| 2026-09-08 | Ver0.17 | 終了BEEP・停止処理・編集制限・設定保存往復・補間表示を修正。MP3出力を削除。回帰テスト9件成功、Chromeで公開版を確認。停止後の表示残りは保留。 |
| 2026-06-07 | Ver0.16 | localStorageによる設定自動保存と、設定コードのコピー/貼り付け読み込みを追加。設定データにschemaVersionを持たせ、将来の項目追加に備える。 |
| 2026-06-07 | Ver0.15 | 回数（回/分）を小数第1位まで指定可能に変更。BPM表示・タイムライン表示・TTS読み上げも小数対応。 |
| 2026-06-07 | Ver0.14 | 全体時間・セグメント終了時刻・時間アナウンスの上限を1200秒へ拡張。MP3出力の長時間負荷は別途検討。 |
| 2026-06-07 | Ver0.12 | 安定版として機能していたことを確認済み。今後の変更時に戻れる基準点として扱う。 |

---

## Ver0.17 検証記録（2026-09-08）

### 実装の更新

- `jumpsAt()` を再生間隔と表示で共用。
- 終了BEEPをビートのwhileループから独立させ、終了予定時刻を先読みして予約。
- `trackSource()` で音源を追跡し、停止時に停止・切断。終了タイマーと再生識別子により前回の終了通知を無効化。
- 開始アナウンスのタイマーとコールバックも再生識別子で管理。
- 再生用の設定をコピーし、`syncEditingLock()` で再生中の設定編集を無効化。
- 時間アナウンスの読込上限を入力・保存と同じ1200秒に統一。
- MP3ボタン、ステータス、翻訳、CSS、`export.js`、lamejs読み込みを削除。

### 確認した範囲

`node --test tests/regression.cjs` は9件成功。JavaScript構文チェック・差分チェックも成功。

[公開ページ](https://ok-rope.github.io/Speed_Practice_App/)のChrome検証では、10秒・2セグメント（80→60回/分）の設定を使用した。再生中の編集制限、言語切替後の制限維持、停止時の制限解除、停止直後の再再生、自然終了、補間BPM表示を確認。全体10秒に対する時間アナウンス60秒が再読込後も維持された。コンソールのerror/warnを検出せず、設定は検証後に復元した。

終了BEEPの予約時刻はモックで確認したが、ミリ秒単位の実音精度は計測していない。iOS/Android実機確認も未実施。

### 既知の不具合：停止後に再生情報が残る（保留）

- 自然終了・手動停止後に、BPM・残り時間・セグメント表示が残る。
- Chromeで `#playInfo.hidden === true`、計算済みスタイル `display: flex` を確認。`.play-info` のCSSがhiddenによる非表示を上書きしている。
- 対応候補は `.play-info[hidden] { display: none; }`。この修正は未実施。
- ユーザーは現状で一区切りとし、今回はMD更新のみを指示。

## 過去の開発記録について

以下の第1〜15節は過去の実装・調査メモを含む履歴。旧MP3仕様、旧終了BEEPの時刻クランプ、旧補間コードは現行実装ではない。ブラウザ依存の説明も今回の実機検証結果とは区別する。現行仕様は [spec.md](spec.md) を参照。
...
*作成日: 2026-05-09*
*更新日: 2026-09-08（Ver0.17の実装・検証結果・保留事項を記録）*
